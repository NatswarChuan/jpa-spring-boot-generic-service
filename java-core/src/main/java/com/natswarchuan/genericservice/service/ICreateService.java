 ...
 * <p>
 * Cung cấp các phương thức để tạo mới thực thể từ Entity hoặc DTO, hỗ trợ trả
 * về Entity hoặc DTO.
 * Cho phép can thiệp vào quá trình tạo thông qua các hook
 * {@link #beforeCreate(Object)} và {@link #afterCreate(Object)}.
 *
...
public interface ICreateService<E, ID> extends IBaseService<E, ID> {
     ...
     * Quy trình:
     * <ol>
     * <li>Gọi {@link #beforeCreate(Object)} để xử lý trước khi lưu.</li>
     * <li>Lưu thực thể vào cơ sở dữ liệu.</li>
     * <li>Gọi {@link #afterCreate(Object)} để xử lý sau khi lưu.</li>
     * </ol>
     *
     ...
     * <p>
     * Chuyển đổi DTO sang Entity và thực hiện quy trình tạo mới chuẩn.
     *
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    default <D extends IDto<E>, S extends IDto<E>> D create(@NonNull S newEntity, @NonNull Class<D> dtoClass) {
        E entity = newEntity.toEntity();
        if (entity == null) {
            return null;
        }
     ...
     * <p>
     * Tương tự {@link #create(IDto, Class)} nhưng đảo ngược thứ tự tham số để thuận
     * tiện sử dụng.
     *
     * @param <S>      Kiểu DTO kết quả.
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    default <S extends IDto<E>, T extends IDto<E>> S create(@NonNull Class<S> dtoClass, @NonNull T dto) {
    ...
    /**
     * Hook được gọi trước khi thực thể được lưu vào cơ sở dữ
     * liệu.
     * <p>
     * Có thể ghi đè để thực hiện validate, thiết lập giá trị mặc định, hoặc sửa đổi
     * dữ liệu.
     *
    ...
    /**
     * Hook được gọi sau khi thực thể đã được lưu thành công vào
     * cơ sở dữ liệu.
     * <p>
     * Có thể ghi đè để thực hiện các tác vụ phụ như gửi email, ghi log, hoặc cập
     * nhật cache.
     *
