 ...
 * <p>
 * Cung cấp các phương thức để cập nhật thực thể từ Entity hoặc DTO.
 * Hỗ trợ các hook {@link #beforeUpdate(Object)} và {@link #afterUpdate(Object)}
 * để can thiệp vào quá trình lưu.
 *
...
public interface IUpdateService<E, ID> extends IBaseService<E, ID> {
     ...
     * Quy trình:
     * <ol>
     * <li>Kiểm tra sự tồn tại của thực thể theo ID (ném lỗi nếu không tìm
     * thấy).</li>
     * <li>Gọi {@link #beforeUpdate(Object)} để xử lý logic trước khi lưu.</li>
     * <li>Lưu thực thể vào cơ sở dữ liệu.</li>
     * <li>Gọi {@link #afterUpdate(Object)} để xử lý sau khi lưu.</li>
     * </ol>
     *
     ...
     * @throws com.natswarchuan.genericservice.exception.HttpException Nếu không tìm
     *                                                                 thấy thực thể
     *                                                                 với ID cung
     *                                                                 cấp.
     */
     ...
     * <p>
     * Gọi hook xử lý trước khi lưu, thực hiện lưu và gọi hook sau khi lưu.
     *
     ...
     * <p>
     * Tương tự như {@link #update(Object)}, thường dùng cho cả thao tác tạo mới và
     * cập nhật trong JPA.
     *
     ...
     * <p>
     * Tìm thực thể cũ theo ID, cập nhật dữ liệu từ DTO vào thực thể đó, sau đó lưu
     * lại.
     *
     ...
     * <p>
     * Chuyển đổi DTO đầu vào sang Entity, thực hiện cập nhật, sau đó chuyển đổi
     * Entity kết quả sang DTO đích.
     *
     ...
     * <p>
     * Tìm thực thể cũ, map dữ liệu từ Request DTO, lưu và map sang Response DTO.
     *
    ...
    @Transactional(rollbackFor = Exception.class)
    default <S extends IDto<E>> S update(@NonNull E updateEntity, @NonNull ID id, @NonNull Class<S> dtoClass) {
        findByIdInternal(id);
        E entity = beforeUpdate(updateEntity);
        if (entity == null) {
            return null;
     ...
     * <p>
     * Lưu ý: Phương thức mặc định này tìm tất cả thực thể theo spec, gọi hook
     * {@code beforeUpdate} trên từng phần tử (bỏ qua tham số {@code updateEntity}),
     * sau đó lưu lại.
     *
     * @param updateEntity Thực thể mẫu (có thể không được dùng trong implementation
     *                     mặc định).
     ...
     * <p>
     * Có thể ghi đè để thực hiện validate, tính toán lại giá trị, hoặc cập nhật
     * trường audit (updatedAt, updatedBy).
     *
    ...
    /**
     * Hook được gọi sau khi thực thể đã được lưu/cập nhật thành
     * công.
     * <p>
     * Có thể ghi đè để thực hiện các tác vụ phụ như clear cache, ghi log, gửi thông
     * báo.
     *
