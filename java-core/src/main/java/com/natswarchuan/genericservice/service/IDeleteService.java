 ...
 * <p>
 * Cung cấp các phương thức xóa theo ID, xóa theo Entity, xóa trả về DTO, và xóa
 * hàng loạt.
 * Hỗ trợ các hook {@link #beforeDelete(Object)} và {@link #afterDelete(Object)}
 * để can thiệp vào quá trình xóa.
 *  
...
public interface IDeleteService<E, ID> extends IBaseService<E, ID> {
     ...
     * <p>
     * Tìm thực thể theo ID, gọi hook xử lý trước khi xóa, thực hiện xóa và gọi hook
     * sau khi xóa.
     *
     ...
     * <p>
     * Gọi hook xử lý trước khi xóa, thực hiện xóa và gọi hook sau khi xóa.
     *
     ...
     * <p>
     * Hữu ích khi cần trả về thông tin của đối tượng vừa bị xóa cho client.
     *
     ...
     * <p>
     * Chuyển đổi DTO đầu vào sang Entity, thực hiện xóa, sau đó chuyển đổi Entity
     * đã xóa sang DTO kết quả.
     *
     * @param <S>      Kiểu DTO trả về.
     * @param <T>      Kiểu DTO đầu vào.
     * @param dtoClass Lớp DTO đích để chuyển đổi dữ liệu trả về.
     * @param dto      Đối tượng DTO chứa thông tin thực thể cần xóa. Không được
     *                 null.
     * @return Đối tượng DTO chứa dữ liệu của thực thể đã xóa.
    ...
    @Transactional(rollbackFor = Exception.class)
    default <S extends IDto<E>, T extends IDto<E>> S delete(@NonNull Class<S> dtoClass, @NonNull T dto) {
     ...
     * <p>
     * Tìm thực thể theo ID, xóa nó và trả về thông tin đã xóa.
     *
     * @param <S>      Kiểu DTO trả về.
     * @param id       ID của thực thể cần xóa. Không được null.
     * @param dtoClass Lớp DTO đích để chuyển đổi dữ liệu.
     ...
     * <p>
     * Tìm tất cả thực thể khớp với {@code spec}, áp dụng hook {@code beforeDelete}
     * cho từng thực thể, xóa hàng loạt, sau đó áp dụng hook {@code afterDelete}.
     *
     * @param spec Điều kiện lọc (Specification) để xác định các thực thể cần xóa.
     *             Không được null.
     */
    @Transactional(rollbackFor = Exception.class)
     ...
     * <p>
     * Có thể ghi đè để thực hiện các kiểm tra logic, validate, hoặc cập nhật dữ
     * liệu liên quan trước khi xóa.
     *
     ...
     * <p>
     * Có thể ghi đè để thực hiện các tác vụ dọn dẹp, ghi log, hoặc cập nhật cache.
     *
