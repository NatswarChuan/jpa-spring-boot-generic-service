 ...
 *
 * <p>Cung cấp các phương thức để truy vấn danh sách thực thể, đếm số lượng, phân trang, và chuyển
 * đổi kết quả sang DTO. Hỗ trợ các hook {@link #afterReadEntity(List)} và {@link
 * #afterReadDto(List)} để xử lý dữ liệu sau khi đọc.
 *
...
public interface IReadSummaryService<E, ID> extends IBaseService<E, ID> {
   ...
   *
   * <p>Kết quả sẽ được xử lý qua hook {@link #afterReadEntity(List)}.
   *
   ...
   *
   * @param ids Tập hợp các ID cần tìm. Nếu null hoặc rỗng sẽ trả về danh sách rỗng.
  ...
  default <S extends IDto<E>> List<S> findAll(Class<S> dtoClass) {
    if (dtoClass == null) {
      throw new HttpException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Class của DTO không được xác định");
    }
  ...
  /**
   * Lấy danh sách thực thể theo ID và chuyển đổi sang DTO (có hỗ trợ đa ngôn ngữ).
   *
   * @param <S> Kiểu DTO trả về.
   * @param ids Danh sách ID cần tìm.
   * @param dtoClass Lớp DTO đích.
   ...
   */
  @Transactional(readOnly = true)
  default <S extends IDto<E>> List<S> findAllById(
      Collection<ID> ids, Class<S> dtoClass, String language) {
    if (ids == null || ids.isEmpty()) {
      return Collections.emptyList();
    }
    if (dtoClass == null) {
  ...
  /**
   * Lấy danh sách thực thể phân trang và chuyển đổi sang trang DTO (có hỗ trợ đa ngôn ngữ).
   *
   * @param <S> Kiểu DTO trả về.
   * @param page Số thứ tự trang.
   * @param size Kích thước trang.
   ...
   */
  @Transactional(readOnly = true)
  default <S extends IDto<E>> Page<S> findAll(
  ...
  /**
   * Lấy danh sách thực thể phân trang theo tiêu chí và chuyển đổi sang trang DTO (có hỗ trợ đa ngôn
   * ngữ).
   *
   * @param <S> Kiểu DTO trả về.
   * @param page Số thứ tự trang.
  ...
  /**
   * Lấy danh sách thực thể phân trang (dùng Pageable) theo tiêu chí và chuyển đổi sang trang DTO.
   *
   * @param <S> Kiểu DTO trả về.
  ...
  @Transactional(readOnly = true)
  default <S extends IDto<E>> Page<S> findAll(
      Pageable pageable, Specification<E> spec, Class<S> dtoClass) {
    if (dtoClass == null) {
      throw new HttpException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Class của DTO không được xác định");
    }
   ...
   *
   * <p>Có thể ghi đè để xử lý hoặc lọc thêm danh sách thực thể trước khi trả về.
   *
   ...
   *
   * <p>Có thể ghi đè để xử lý dữ liệu trong trang trước khi trả về.
   *
   ...
   *
   * <p>Có thể ghi đè để bổ sung dữ liệu tính toán hoặc thông tin phụ cho các DTO.
   *
