 ...
 * <p>
 * Cung cấp các phương thức tìm kiếm thực thể duy nhất theo ID hoặc
 * Specification (tiêu chí).
 * Hỗ trợ chuyển đổi sang DTO và các hook xử lý sau khi đọc
 * {@link #afterReadEntity} và {@link #afterReadDto}.
 *
...
public interface IReadDetailService<E, ID> extends IBaseService<E, ID> {
     ...
     * <p>
     * Nếu tìm thấy, thực thể sẽ được xử lý qua hook
     * {@link #afterReadEntity(Object)}.
     *
    ...
    default E findById(@NonNull ID id) {
        E result = getRepository().findById(id)
                .orElseThrow(() -> new HttpException(HttpStatus.NOT_FOUND, "Không tìm thấy thực thể với id= " + id));
     ...
     * @return Thực thể tìm thấy.
     * @throws HttpException Nếu không tìm thấy thực thể thỏa mãn tiêu chí (HTTP
     *                       404).
     */
    @Transactional(readOnly = true)
    default E findOne(@NonNull Specification<E> spec) {
        E result = getRepository().findOne(spec)
                .orElseThrow(() -> new HttpException(HttpStatus.NOT_FOUND,
                        "Không tìm thấy thực thể với tiêu chí cung cấp."));
        return afterReadEntity(result);
    }
    ...
    default <S extends IDto<E>> S findById(@NonNull ID id,@NonNull Class<S> dtoClass, String language) {
        E result = getRepository().findById(id)
                .orElseThrow(() -> new HttpException(HttpStatus.NOT_FOUND, "Không tìm thấy thực thể với id= " + id));
        E afterReadEntityResult = afterReadEntity(result);
        if (afterReadEntityResult == null) {
    ...
    @Transactional(readOnly = true)
    default <S extends IDto<E>> S findById(@NonNull Specification<E> spec, @NonNull Class<S> dtoClass) {
    ...
    /**
     * Tìm thực thể dựa trên tiêu chí bổ sung (Specification) và trả về DTO (hỗ trợ
     * ngôn ngữ).
     * <p>
     * Phương thức này sử dụng Specification để tìm kiếm.
     *
     * @param <S>      Kiểu DTO trả về.
    ...
    @Transactional(readOnly = true)
    default <S extends IDto<E>> S findById(@NonNull ID id, @NonNull Class<S> dtoClass, @NonNull Specification<E> spec, String language) {
        E result = findOne(spec);
        E afterReadEntityResult = afterReadEntity(result);
        if (afterReadEntityResult == null) {
            return null;
    ...
    @Transactional(readOnly = true)
    default <S extends IDto<E>> S findOne(@NonNull Class<S> dtoClass, @NonNull Specification<E> spec) {
        E result = findOne(spec);
        E afterReadEntityResult = afterReadEntity(result);
        if (afterReadEntityResult == null) {
            return null;
    ...
    @Transactional(readOnly = true)
    default <S extends IDto<E>> S findOne(@NonNull Class<S> dtoClass, @NonNull Specification<E> spec, String language) {
        E result = findOne(spec);
        E afterReadEntityResult = afterReadEntity(result);
        if (afterReadEntityResult == null) {
            return null;
     ...
     * @return {@code true} nếu tồn tại ít nhất một thực thể, ngược lại
     *         {@code false}.
     */
     ...
     * <p>
     * Có thể ghi đè để thực hiện các xử lý bổ sung trên thực thể trước khi trả về
     * hoặc chuyển đổi sang DTO.
     *
     ...
     * <p>
     * Có thể ghi đè để bổ sung dữ liệu cho DTO.
     *
