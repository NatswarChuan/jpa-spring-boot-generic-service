...
public interface IBaseService<E, ID> {
   ...
   *
   * <p>Các lớp triển khai phải override phương thức này để cung cấp repository.
   *
  ...
  default E findByIdInternal(@NonNull ID id) {
    return getRepository()
        .findById(id)
        .orElseThrow(
            () -> new HttpException(HttpStatus.NOT_FOUND, "Không tìm thấy thực thể với id= " + id));
  }
   ...
   * @throws HttpException nếu xảy ra lỗi khi khởi tạo DTO.
   */
  default <S extends IDto<E>> S mapToDto(
      @NonNull E entity, @NonNull Class<S> dtoClass, String language) {
   ...
   * @throws HttpException nếu xảy ra lỗi khi khởi tạo DTO.
   */
  default <S extends IDto<E>> List<S> mapToDtos(
      @NonNull List<E> entities, @NonNull Class<S> dtoClass, String language) {
   ...
   */
  default <S extends IDto<E>> List<S> mapToDtos(
      @NonNull List<E> entities, @NonNull Class<S> dtoClass) {
   ...
   * @throws HttpException nếu xảy ra lỗi khi khởi tạo DTO.
   */
  default <S extends IDto<E>> Page<S> mapToDtos(
      @NonNull Page<E> entities, @NonNull Class<S> dtoClass, String language) {
    return entities.map(
        entity -> {
          if (entity == null) {
            return null;
   ...
   */
  default <S extends IDto<E>> Page<S> mapToDtos(
      @NonNull Page<E> entities, @NonNull Class<S> dtoClass) {
    return mapToDtos(entities, dtoClass, null);
  }
}
