 ...
 *
 * <p>Cung cấp endpoint để lấy thông tin đầy đủ của một bản ghi đơn lẻ.
 *
...
public interface IReadDetailController<E, ID> extends IBaseController<E, ID> {
  ...
  @Operation(
      ...
      description = "Lấy thông tin chi tiết của một bản ghi dựa trên ID.")
  @ApiResponses(
      value = {
  ...
  default ResponseEntity<HttpApiResponse<? extends IDto<E>>> findById(
      ...
      @RequestHeader(name = "Accept-Language", defaultValue = "en", required = false)
          String language) {
    IDto<E> result = getReadDetailService().findById(id, getResponseDetailDtoClass(), language);
   ...
   *
   * @param <S> Kiểu dữ liệu thực tế của Service (kế thừa {@link IReadDetailService}).
   * @return Đối tượng Service.
   * @throws HttpException Nếu Service hiện tại không implement {@link IReadDetailService}.
   */
  @SuppressWarnings("unchecked")
  default <S extends IReadDetailService<E, ID>> S getReadDetailService() {
    if (getBaseService() instanceof IReadDetailService) return (S) getBaseService();
    else
      throw new HttpException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Service không implement IReadDetailService");
  }
}
