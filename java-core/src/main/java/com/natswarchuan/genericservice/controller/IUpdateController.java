 ...
 *
 * <p>Cung cấp endpoint để cập nhật thông tin của một bản ghi đã tồn tại.
 *
 ...
 * @author NatswarChuan
 */
public interface IUpdateController<E, ID, UPDATE_REQ extends IDto<E>>
   ...
   *
   * <p>Dữ liệu trong Payload sẽ được validate và dùng để cập nhật bản ghi có ID tương ứng.
   *
   ...
   * @throws HttpException Nếu Service hiện tại không implement {@link IUpdateService}.
   */
  ...
  default <S extends IUpdateService<E, ID>> S getUpdateService() {
    if (getBaseService() instanceof IUpdateService) {
      ...
      throw new HttpException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Service không implement IUpdateService");
    }
  }
}
