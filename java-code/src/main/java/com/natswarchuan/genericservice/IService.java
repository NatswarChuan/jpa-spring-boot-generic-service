 ...
 *
 * <p>Định nghĩa các thao tác CRUD (Create, Read, Update, Delete) cơ bản và các phương thức tiện ích
...
public interface IService<E, ID> {
   ...
   * @throws HttpException Nếu không tìm thấy thực thể với ID cung cấp, hoặc nếu có lỗi xảy ra trong
   *     quá trình xóa.
   */
  void delete(ID id);

  /**
   * Lấy danh sách tất cả các thực thể và chuyển đổi chúng thành danh sách các DTO.
   *
   * @param <S> Kiểu của DTO, phải triển khai {@link IDto} cho thực thể {@code E}.
   * @param dtoClass Lớp (Class) của DTO. Được sử dụng để tạo các instance mới của DTO.
   * @return Danh sách các DTO tương ứng với tất cả các thực thể. Trả về danh sách rỗng nếu không có
   *     thực thể nào.
   * @throws HttpException Nếu có lỗi xảy ra trong quá trình truy vấn hoặc chuyển đổi.
   */
   ...
   * @param dtoClass Lớp (Class) của DTO. Được sử dụng để tạo instance mới của DTO.
   ...
   *
   * @return Danh sách tất cả các thực thể. Trả về danh sách rỗng nếu không có thực thể nào.
   * @throws HttpException (Mặc dù không được ném trực tiếp trong Javadoc này, các triển khai có thể
   ...
   * @return Đối tượng {@link Page} chứa danh sách các thực thể của trang hiện tại và thông tin phân
   *     trang.
   * @throws HttpException (Mặc dù không được ném trực tiếp trong Javadoc này, các triển khai có thể
   *     ném nếu có lỗi truy cập dữ liệu).
   */
   ...
   * @throws HttpException Nếu có lỗi xảy ra trong quá trình chuyển đổi DTO thành thực thể hoặc
   ...
   *
   * @param <S> Kiểu của DTO, phải triển khai {@link IDto} cho thực thể {@code E}.
   * @param deleteEntity Đối tượng DTO (có thể không được sử dụng trực tiếp nếu chỉ cần ID).
   * @param id Khóa chính (ID) của thực thể cần xóa.
   * @throws HttpException Nếu không tìm thấy thực thể với ID cung cấp, hoặc nếu có lỗi xảy ra trong
   *     quá trình xóa.
   */
   ...
   * @param id Khóa chính (ID) của thực thể cần cập nhật (dùng để kiểm tra sự tồn tại).
   * @throws HttpException Nếu không tìm thấy thực thể với ID cung cấp, hoặc nếu có lỗi xảy ra trong
   *     quá trình cập nhật.
   */
   ...
   * Xóa một thực thể.
   *
   * @param deleteEntity Thực thể cần xóa (có thể không được sử dụng trực tiếp nếu chỉ cần ID).
   * @param id Khóa chính (ID) của thực thể cần xóa (dùng để kiểm tra sự tồn tại).
   * @throws HttpException Nếu không tìm thấy thực thể với ID cung cấp, hoặc nếu có lỗi xảy ra trong
   *     quá trình xóa.
   */
  ...
  /**
   * Tìm tất cả các thực thể dựa trên một tập hợp các khóa chính (IDs) được cung cấp.
   *
   * @param ids Tập hợp các khóa chính (IDs) của các thực thể cần tìm.
   * @return Danh sách các thực thể được tìm thấy. Trả về danh sách rỗng nếu tập hợp {@code ids} là
   *     {@code null}, rỗng, hoặc không tìm thấy thực thể nào.
   */
  List<E> findAllById(Collection<ID> ids);

  /**
   * Lấy danh sách các thực thể theo trang và chuyển đổi chúng thành danh sách các DTO.
   *
   * @param <S> Kiểu của DTO, phải triển khai {@link IDto} cho thực thể {@code E}.
   * @param page Số trang cần lấy (thường bắt đầu từ 0).
   * @param size Số lượng thực thể trên mỗi trang.
   * @param dtoClass Lớp (Class) của DTO. Được sử dụng để tạo các instance mới của DTO.
   * @return Đối tượng {@link Page} chứa danh sách các DTO của trang hiện tại và thông tin phân
   *     trang.
   * @throws HttpException (Mặc dù không được ném trực tiếp trong Javadoc này, các triển khai có thể
  ...
  /**
   * Tạo mới một thực thể từ một DTO và trả về một DTO khác đại diện cho thực thể đã tạo.
   *
  ...
  /**
   * Cập nhật một thực thể với dữ liệu từ một DTO yêu cầu và trả về một DTO phản hồi.
   *
   * @param <RQ> Kiểu của DTO yêu cầu (request), phải triển khai {@link IDto}.
   * @param <RP> Kiểu của DTO phản hồi (response), phải triển khai {@link IDto}.
   ...
   * @return Một DTO phản hồi kiểu {@code RP} đại diện cho thực thể sau khi cập nhật.
   * @throws HttpException Nếu không tìm thấy thực thể hoặc có lỗi xảy ra trong quá trình cập nhật.
   */
  <RQ extends IDto<E>, RP extends IDto<E>> RP update(RQ updateEntity, ID id, Class<RP> rsClass);

  /**
   * Lấy danh sách các thực thể theo trang, có hỗ trợ đa ngôn ngữ và chuyển đổi sang DTO.
   *
  ...
  /**
   * Tìm kiếm và phân trang các thực thể dựa trên các tiêu chí lọc, có hỗ trợ đa ngôn ngữ.
   *
   * @param <S> Kiểu của DTO, phải triển khai {@link IDto}.
   ...
   *
   * @param <S> Kiểu generic không được sử dụng trong phương thức này nhưng có thể cần thiết cho
   ...
   *
   * @param <S> Kiểu của DTO, phải triển khai {@link IDto}.
   * @param id Khóa chính của thực thể (thường được dùng để xây dựng {@code spec}).
   * @param dtoClass Lớp của DTO để chuyển đổi.
   * @param spec Đối tượng {@link Specification} chứa các điều kiện lọc (ví dụ: kết hợp id và các
   *     điều kiện khác).
